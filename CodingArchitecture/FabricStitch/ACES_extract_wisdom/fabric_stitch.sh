...
case "$SOURCE_TYPE" in
  ...
  file)
    cat "$SOURCE_FILE" | \
      fabric --pattern ACES_extract_wisdom \
             --model "$STEP1_MODEL" \
             --vendor "$STEP1_VENDOR" \
             --output "${STAGING}/step-01-extracted-wisdom.md"
    ;;
...
for entry in \
  "${TIMESTAMP_NOW}|fabric_stitch|${RUN_ID}|CodingArchitecture/FabricStitch/ACES_extract_wisdom|fabric_stitch.step_1|${STEP1_VENDOR}|${STEP1_MODEL}|${IN1}|${OUT1}|0|0|${COST1}|${DUR1}|dev|${RUN_ID}|extract_wisdom" \
  "${TIMESTAMP_NOW}|fabric_stitch|${RUN_ID}|CodingArchitecture/FabricStitch/ACES_extract_wisdom|fabric_stitch.step_2|${STEP2_VENDOR}|${STEP2_MODEL}|${IN2}|${OUT2}|0|0|${COST2}|${DUR2}|dev|${RUN_ID}|summarize" \
  "${TIMESTAMP_NOW}|fabric_stitch|${RUN_ID}|CodingArchitecture/FabricStitch/ACES_extract_wisdom|fabric_stitch.step_3|${STEP3_VENDOR}|${STEP3_MODEL}|${IN3}|${OUT3}|0|0|${COST3}|${DUR3}|dev|${RUN_ID}|extract_insights" \
  "${TIMESTAMP_NOW}|fabric_stitch|${RUN_ID}|CodingArchitecture/FabricStitch/ACES_extract_wisdom|fabric_stitch.step_4|${STEP4_VENDOR}|${STEP4_MODEL}|${IN4}|${OUT4}|0|0|${COST4}|${DUR4}|dev|${RUN_ID}|synthesize_narrative" \
  "${TIMESTAMP_NOW}|fabric_stitch|${RUN_ID}|CodingArchitecture/FabricStitch/ACES_extract_wisdom|session.total|all|all|${total_in}|${total_out}|0|0|${total_cost}|${pipeline_ms}|dev|${RUN_ID}|pipeline_complete"
do
  echo "$entry" >> "$COST_LOG"
done
echo ""
